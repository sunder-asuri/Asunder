...
def modify():
    ...
    updateBooks = "update " + bookTable + " set bid = '"+ bid + "',title = '" + title + "',author = '"+ author + "',status = '" + status + "' where bid = '"+ bid + "';"
    try:
        cur.execute(updateBooks)
        con.commit()
        messagebox.showinfo('Success', "Book modified successfully")
    except:
        messagebox.showinfo("Error", "Can't modify data into Database")
...
def modifyBook():
    ...
    headingLabel = Label(headingFrame1, text="Modify Books", bg='black', fg='white', font=('Courier', 15))
    ...
    # Submit Button
    SubmitBtn = Button(root, text="SUBMIT", bg='#d1ccc0', fg='black', command=modify)
